...
Asynchous, or "self-timed", logic admits an area overhead for inter-stage control, but in return provides some
benefits, including:
...
- only dynamically involved with the computation toggles, potentially saving power,
...
### Request and Acknoledge

For historical reasons we denote data validity with "request" and stage readiness is signalled by the
receiving stages with the acknowledge signal, denoting that data have been recieved (and latched).
For well defined behavior it is required that both of request can only transition once acknowledge have
transitioned (and vise versa). This means that the request/acknowledge pair will cycle though 00, 10, 11,
and 01, and only these values.

### Delays

It is essential that data are stable before request is asserted.  This is achieved via a delay must be longer
...
usecase and due to time-constraints we had to guess at sufficient delays.  This is the major risk in this
...
Not so in asynchrous logic which each state is effectively its own clock domain.  Thus when we join values
from multiple paths with must use special synchronizers.  This is true for forks as well as we can't proceed
...
running sequence).  We use 26-bits of internal precision, but we only
have 15-bits for outputs, we what is actually emitted is `r ^ (r >> 15)`.
